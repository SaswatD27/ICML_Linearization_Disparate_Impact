...
def get_architecture(arch: str, dataset: str, device, args) -> torch.nn.Module:
    ...
    elif arch == "resnet18_in" and dataset.lower() == "utkface_age":
        model = resnet18_in(num_classes=5, args=args).to(device)
    elif arch == "resnet18_in" and dataset.lower() == "utkface_race":
        model = resnet18_in(num_classes=5, args=args).to(device)
    elif arch == "resnet18_in" and dataset == "cifar10":
        model = resnet18_in(num_classes=10, args=args).to(device)
    elif arch == "resnet18_in" and dataset.lower() == "celeba":
        model = resnet18_in(num_classes=8, args=args).to(device)
    elif arch == "resnet18_in" and dataset.lower() == "celeba_unbalanced":
        model = resnet18_in(num_classes=8, args=args).to(device)
